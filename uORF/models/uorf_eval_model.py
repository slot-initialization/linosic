...
from .model import Encoder, Decoder, SlotAttention, raw2outputs, SlotAttentionRegular, SlotAttentionOwn
...
class uorfEvalModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--slot_init', type=str, default="kmeans", help='What kind of slot initialization do you want: PseudoWeights, kmeans, pooling, pixelattn, vectorattn, kmeansexp, hash, SAMLPMax, SA5CC, SAMLPSum, SAMLPMaxRes, SAMLPSumRes, SAPoolingMax, SAPoolingSum, HashMax, HashSum, SAHash, SAHashMax, SAHashMaxReLU, SAHashSum, regular, own')  # hashparam, hashparamself,
    ...
    def __init__(self, opt):
        ...
        if opt.slot_init == 'regular':
            ...
            self.netSlotAttention = networks.init_net(
                SlotAttention(opt=opt, num_slots=opt.num_slots, in_dim=z_dim, slot_dim=z_dim, iters=opt.attn_iter),
                gpu_ids=self.gpu_ids, init_type='normal')
    ...
    def forward(self, epoch=0):
        ...
        for (j, (frus_nss_coor_, z_vals_, ray_dir_)) in enumerate(zip(frus_nss_coor, z_vals, ray_dir)):
            ...
            raws_, masked_raws_, unmasked_raws_, masks_ = self.netDecoder(sampling_coor_bg_[None, ...], sampling_coor_fg_[None, ...], z_slots[None, ...], nss2cam0[None, ...])  # (NxDxHxW)x4, Kx(NxDxHxW)x4, Kx(NxDxHxW)x4, Kx(NxDxHxW)x1
            #print(raws_)
            ...
            rgb_map_, depth_map_, _ = raw2outputs(raws_[None, ...], z_vals_[None, ...], ray_dir_[None, ...])
            # (NxHxW)x3, (NxHxW)
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            for k in range(self.num_slots):
                ...
                #print(z_vals.sum(), ray_dir.sum(), raws.sum())
                raws = raws.permute([0, 2, 3, 1, 4]).flatten(start_dim=0, end_dim=2)  # (NxHxW)xDx4
                rgb_map, depth_map, _, mask_map = raw2outputs(raws[None, ...], z_vals[None, ...], ray_dir[None, ...], render_mask=True)
